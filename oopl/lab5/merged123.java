...
class student {
    ...
    public static void substr(student[] record, String sub,int len){
        System.out.println("names with "+sub+":\n");
        for (int i =0;i<len;i++){
            if(record[i].fName.contains(sub)){
                record[i].display();
    ...
    // change full name to initials
    public static void chgin(student[] record,int len){
        System.out.println("student names as initials: ");
        for (int i =0;i<len;i++){
            if(record[i].fName!=""){
                String[] fName = record[i].fName.split(" ");
                String res = "";
    ...
    public static void main(String args[]) {
        ...
        while (true){
            ...
            if (input == 1) {
                ...
                System.out.println("enter the substring to find the students with that substring: ");
                String sub = sc.next();
                student.substr(stds, sub, stds.length);
            }

            else if (input == 5){
                student.chgin(stds, stds.length);                
            }
